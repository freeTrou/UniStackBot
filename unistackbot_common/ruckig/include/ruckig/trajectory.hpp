...
class Trajectory {
    ...
    // [UniStackBot RT 补丁 2026-09-18] 原版 SetIntegrate = std::function, 每次调用堆分配
    // 32B (lambda 捕获集超 SBO 上限) —— 违反 update() 零分配。模板化回调消除分配,
    // 语义不变。上游 issue 待提; 版本前进时此处需重查。
    template <typename SetIntegrateFn>
    void state_to_integrate_from(double time, size_t& new_section, SetIntegrateFn&& set_integrate) const {
        if (time >= duration) {
