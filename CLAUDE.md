...
- **unistackbot_interface** — 公共接口定义包（ROS msg/srv/action + 纯 C++ 共享契约头）：跨包/跨仓库共享的类型放在这里（单一事实源，供算法团队外部仓库依赖）。已落地：① 机器人级契约（2026-09-17 定稿）：`RobotFeedback`/`RobotCommand`（含冗余偏好与 MIT 模式；模式/状态两正交维度）、`IkResult`/`SimResult` 分层结果码（错误跟层走、设备状态住反馈帧、公共失败码取消）、`joint_capacity.hpp` 的 kMaxJoints 唯一定义；② `/sim_control` 契约**不住本包**——住 `unistackbot_sim_control`（2026-09-17 终局：实现归实现的家、类型归类型的家、契约归契约的主人）。
