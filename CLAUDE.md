...
- **unistackbot_demo** — 控制 demo 独立包（2026-09-23 从 bringup/scripts 独立）: 三命令域演示程序（§16.2 透传原则）= 链路契约的独立外部消费者（关节表/限位/mimic 自持解析 URDF、频率定源=配置文件直读、零机型硬编码）。① `demo_cartesian.py` 末端位姿流: 四 pattern `up`(z 往返, `--traverse` 插值流 @`--stream-hz`) / `axes`(抬升基准+8 向 2cm 逐点扫描, 诚实拒绝记"拒"不判死) / `sweep`(大幅: 抬升+前伸部署→r10cm 圆周 16 点逐点快慢交替→收回, `--lift/--fwd/--span/--speed`×每腿系数 慢0.5/快1.6, 总程 >20s; piper 探针: 零位仅 +z 可行, 部署位邻域 ±150mm 球全解, 活链 22/22 腿) / `reject`(负路径验证: 不可达弹幕逐点断言被拒+恢复目标照常收敛——诚实拒绝也是验收面, mock 4/4 覆盖两结果码)。② `demo_motion.py --hz 50` 关节慢流（JS ruckig 档填充; 默认运动=四段快慢大幅序列 85%→15%→85%→零位 ~14.5s, 快慢腿速度对比 ~3×）。③ `demo_joint_fullrate` **C++** 关节满速流（=控制频率, hold 退化=透传; **python rclpy 500Hz 全抖动故 C++**; rclcpp wall 定时器下发; 默认运动=四段快慢大幅序列同②口径 (`duration:=` 总时长, 默认 14.5s, 各段按占比缩放); 频率定源=**bringup config `/**.update_rate` 配置文件直读** (2026-09-24 终版, 免参数服务), 多份 *_controllers.yaml 时 `-p robot:=` 必填 fail-fast 列选项, stamp 差分中位数仅末位兜底(高流被 2ms 轮询欠采样低读); **launch `bus_hz:=` 运行期覆盖文件读不到——满速 demo 对 1000Hz 档须显式 `-p hz:=1000.0`**; mock 实测 500.0Hz/99.9% 达成, 结束如实报告名义/实际+来源）。
- **unistackbot_controller** — 控制器集成 + ROS 工具节点；算法库住 `unistackbot_algorithm`（2026-09-17 拆分），本包 `<depend>unistackbot_algorithm</depend>` 只消费类型。两个自研控制器:
  ...
  - **OtgGateController —— 已退役**（2026-09-23 落地, **2026-09-24 删代码**）: §16.6 语义统一后其通路（订阅回调线程一次 IK → RT 环 OtgStream 输出级）**就是 CM 的新实现**, 独立控制器删除（代码/CMake/两 yaml 段）; 终点命令直接发 CM `~/target`。决策与架构沿革见设计 §16.4/§16.6; `controller_common/urdf_command_joints.hpp` 留存（JS 消费）。
...
- `architecture/hardware_framework_design.md` — master working doc for the real-hardware layer (`unistackbot_hardware` + RT core): 分层架构、线程模型、协议后端契约（master.hpp 五要素、形态盲对象模型）、v3 语义状态机、形态与算法边界（§14）、与 ros2_control 生态的对照审核（§15）、多臂与调度（§17）. Status: 讨论中、未收口 (2026-09).
- `architecture/real_hardware_architecture.md` — **真机架构定稿视图** (2026-09-24 从母文档分叉): 多总线×多臂的七条设计原则、四轴分层（机器=配置/协议=内芯/总线=骨架/状态机=翻译）、调度阶梯（EC95>CAN90>串口85>CM80）与核数哲学（1-2 隔离核、单核≤2总线）全部 bench 实测背书、双臂控制形态（市场模式②+接口统一"身份在消息里"）、安全/错误面、决策索引（定稿 vs 讨论稿分列）。新会话了解真机层架构先读此文件。
...
- `bus/socketcan_master_design.md` / `bus/ethercat_master_design.md` / `bus/rs485_master_design.md` — full designs of the three bus-master backends (SocketCAN CAN FD; IgH ecrt, 1kHz + DC; 串口 RS-485 骨架×协议内芯分层——帧泵/FrameSpec/能力声明, R1-7A 首个消费者), structurally parallel, all written against the master.hpp contract.
